...
async function findDebtCycle(debtor, creditor) {
    return await doBFS(debtor, creditor, async (node) => {
        const neighbors = [];
        const users = await getUsers();
        for (let user of users) {
            let amountOwed = await BlockchainSplitwise.lookup(node, user);
            if (amountOwed > 0) {
                neighbors.push(user);
            }
        }
...
async function resolveCycle(cycle) {
    let minDebt = Infinity;
    for (let i = 0; i < cycle.length - 1; i++) {
        let debtor = cycle[i];
        let creditor = cycle[i + 1];
        let amount = await BlockchainSplitwise.lookup(debtor, creditor);
        minDebt = Math.min(minDebt, parseInt(amount.toString()));
    }

    for (let i = 0; i < cycle.length - 1; i++) {
        let debtor = cycle[i];
        let creditor = cycle[i + 1];
        let contractWithSigner = BlockchainSplitwise.connect(provider.getSigner(debtor));
        await contractWithSigner.add_IOU(creditor, -minDebt);
    }
}
...
async function getUsers() {
    ...
    for (let call of calls) {
        usersSet.add(call.from); // Debtor
        usersSet.add(call.args[0]); // Creditor
    }
...
async function getTotalOwed(user) {
    ...
    for (let creditor of users) {
        let amount = await BlockchainSplitwise.lookup(user, creditor);
        totalOwed += parseInt(amount.toString());
    }
    return totalOwed;
}
...
async function getLastActive(user) {
    var calls = await getAllFunctionCalls(contractAddress, "add_IOU");
    var lastActiveTime = 0; // Default to 0 instead of null

    for (var call of calls) {
        if (call.from.toLowerCase() === user.toLowerCase() || call.args[0].toLowerCase() === user.toLowerCase()) {
            lastActiveTime = Math.max(lastActiveTime, call.t); // Update with latest timestamp
        }
    }

    return lastActiveTime || null; // Return null if no transactions found
}
...
async function add_IOU(creditor, amount) {
    let cycle = await findDebtCycle(defaultAccount, creditor);
    if (cycle) {
        await resolveCycle(cycle);
    }
    let signer = provider.getSigner();
    let contractWithSigner = BlockchainSplitwise.connect(signer);
    await contractWithSigner.add_IOU(creditor, amount);
}
